...
- **(F) Chất lượng gen TC — thứ tự + số lượng + suite** (user phản ánh TC "không đi từ tổng quan→chi tiết, hơi ít case"): (1) **Thứ tự**: `sortTestCases` (`testcase-quality.js`) trước sort theo **name (alphabet)** → đẩy "Boundary..." lên trước "Happy Path...", phá luồng logic. Sửa: GIỮ NGUYÊN thứ tự AI sinh, chỉ gom TC cùng module (theo thứ tự module xuất hiện lần đầu), KHÔNG sort alphabet. (2) **Prompt** (`skill-registry.js` buildPrompt testcase): thêm quy tắc THỨ TỰ tổng quan→chi tiết (happy path/Positive trước → Negative → Boundary → Edge → Security → UI/UX, nhóm cùng kịch bản liền nhau). (3) **Số lượng**: bỏ "Tối đa 20", đổi sang **coverage-driven không giới hạn cứng** (cover hết mọi nhánh, không nhồi trùng) — user chọn hướng này. (4) **Suite**: chỉ là cột metadata (không điều khiển logic; gating dùng `stage`); thêm giải thích ý nghĩa Smoke/Regression/New Feature/Exploratory vào prompt để AI gán đúng (happy path=Smoke, còn lại=Regression).

- **(G) Tối ưu token + Import TC (0 token)** (user phản ánh dùng chút đã hết quota Gemini free-tier): (1) **Mức độ phủ TC**: thêm option `coverage` (fast ~10 / balanced ~20 mặc định / full không giới hạn) ở `useSkillWorkspace` + dropdown trong `SkillOptions`; `buildPrompt` testcase đổi câu SỐ LƯỢNG theo coverage. Giảm token so với "full unlimited" trước đó. (2) **Bổ sung TC**: `appendTestCases` chỉ gửi `{id,name,module}` của TC đã có (thay full JSON) → giảm mạnh input token. (3) **Import TC (0 token, KHÔNG gọi AI)**: `parsePastedTestCases` (`testcase-parser.js`) — parse bảng dán từ Excel/Sheets/Lark/CSV (tokenizer nhận ngoặc kép, tự dò tab/phẩy, map header EN/VI hoặc thứ tự cột cố định khớp `toCsv`); `ImportTestCaseModal.jsx` mới; nút "Import TC (0 token)" cạnh Generate; `handleImportTestCases` MERGE existing+imported → `saveTestCasesApi` (server tự gán ID). (4) SRS vòng hỏi thêm: KIỂM TRA thấy đã tối ưu sẵn (previousSrs là hộp câu hỏi nhỏ, full SRS chỉ ở vòng chốt) → không sửa. Verify parser qua esbuild: TSV/CSV/no-header đều đúng.

- **(H) Sửa lỗi Gemini "high demand" + XOAY VÒNG NHIỀU KEY** (user chỉ có Gemini, không có provider khác để fallback): `gemini.provider.js` viết lại: (1) `callGeminiSingle` — 4 model fallback (`2.5-flash → 2.0-flash → 1.5-flash → flash-latest`) + **retry cùng model với backoff** (1.5s→3s, 3 lần/model) vì "high demand" là spike tạm thời; quota → ném `QUOTA_EXCEEDED` ngay. (2) `callGemini` wrapper — `parseKeys` tách chuỗi key thành NHIỀU key (mỗi dòng/phẩy/chấm phẩy 1 key, dedupe); thử lần lượt, **xoay sang key kế khi QUOTA_EXCEEDED / quá tải kéo dài / key chết (API key not valid, PERMISSION_DENIED)**; lỗi content (safety...) ném ngay không phí key. Chữ ký `callGemini(...)` giữ nguyên. (3) UI `ProviderSettingsModal`: ô Gemini key đổi thành **textarea nhiều dòng** (mỗi dòng 1 key). Lưu qua `saveProviderSetting` (encrypt nguyên chuỗi). **CẦN RESTART BACKEND.**

- **(I) Fix "no longer available" + QUOTA_EXCEEDED sai** (user có key đời mới CHỈ dùng được gemini-3): Probe key thật → `gemini-2.5-flash`/`2.5-flash-lite` trả **404 "no longer available to new users"**, `2.0-flash` **429 quota**, `flash-latest` **503**; chỉ **`gemini-3-flash-preview` + `gemini-3.1-flash-lite`** chạy (JSON OK, finishReason=STOP). (1) Đổi MODELS list ưu tiên gemini-3 rồi mới 2.x: `['gemini-3-flash-preview','gemini-3.1-flash-lite','gemini-2.5-flash','gemini-2.0-flash','gemini-flash-latest']` — model 404 bị skip nhanh sang model kế. (2) **BUG QUAN TRỌNG đã sửa**: trước đây gặp quota 1 model là ném `QUOTA_EXCEEDED` NGAY (thoát vòng model) → không thử model khác dù quota tính RIÊNG từng model. Giờ quota 1 model → `sawQuota=true; break` sang model kế; chỉ ném `QUOTA_EXCEEDED` khi HẾT SẠCH model. E2E test `callGemini` với key thật → trả JSON OK. **RESTART BACKEND.**

### 2. File đã sửa (phiên này)
- `src/server/ai/providers/gemini.provider.js` — MODELS ưu tiên gemini-3; fix per-model quota (không short-circuit); retry backoff; xoay vòng nhiều key. **RESTART BACKEND.**
- `src/web/components/providers/ProviderSettingsModal.jsx` — ô Gemini key = textarea nhiều key.
- `src/web/features/skills/skill-registry.js` — prompt SRS #2 + `buildFinalizePrompt` (nhiều vòng); prompt testcase: openQuestions kèm "Gợi ý:", thứ tự tổng quan→chi tiết, SỐ LƯỢNG theo `coverage`, giải thích suite.
- `src/web/state/useSkillWorkspace.js` — thêm `coverage:'balanced'`.
- `src/web/components/controls/SkillOptions.jsx` — dropdown Độ phủ.
- `src/web/features/testcase/testcase-parser.js` — `parsePastedTestCases` (import bảng, 0 token).
- `src/web/components/output/ImportTestCaseModal.jsx` — MỚI.
- `src/web/features/testcase/testcase-quality.js` — `sortTestCases` giữ thứ tự AI, chỉ gom theo module (bỏ sort alphabet theo name).
