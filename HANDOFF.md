...
- **(H) Sửa lỗi Gemini "high demand" tái diễn**: `gemini.provider.js` trước chỉ 2 model + retry ĐÚNG 1 lần (đổi model, không thử lại) → khi cả hai quá tải thì ném lỗi. Viết lại: 4 model fallback (`gemini-2.5-flash → 2.0-flash → 1.5-flash → flash-latest`) + **retry cùng model với backoff** (1.5s→3s, tối đa 3 lần/model) vì "high demand" là spike tạm thời; quota (429/RESOURCE_EXHAUSTED) ném `QUOTA_EXCEEDED` ngay để router fallback provider khác. **LƯU Ý**: chỉ có Gemini key → không có provider để fallback; retry giúp né spike ngắn nhưng KHÔNG giải quyết quá tải kéo dài → khuyến nghị user thêm key Claude/GPT trong Settings. **CẦN RESTART BACKEND.**

### 2. File đã sửa (phiên này)
- `src/server/ai/providers/gemini.provider.js` — 4 model fallback + retry backoff cho lỗi high-demand/503. **RESTART BACKEND.**
