            ...
            <div className="provider-card">
              <div className="provider-card-header">
                ...
                <div className="provider-info">
                  <span className="provider-name">Gemini API Key</span>
                  <span className="provider-tag">{form.gemini.hasKey ? 'Đã có key trên server' : 'Chưa có key'}</span>
                </div>
              </div>
              <div className="provider-key-row">
                <input className="pf-input" type="password" value={form.gemini.key} onChange={e => update('gemini', { key: e.target.value })} placeholder={form.gemini.hasKey ? 'Để trống để giữ key cũ' : 'Nhập API key'} />
              </div>
            </div>
          )}

          <button className="btn-secondary" type="button" onClick={() => setShowLarkConfig(v => !v)}>
