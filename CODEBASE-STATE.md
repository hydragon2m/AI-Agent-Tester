...
- **Thứ tự + số lượng + suite (2026-07-10)**: `sortTestCases` (testcase-quality.js) GIỮ NGUYÊN thứ tự AI sinh (tổng quan→chi tiết), chỉ gom theo module — KHÔNG sort alphabet theo name. Prompt testcase: SỐ LƯỢNG theo option `coverage` (fast ~10 / balanced ~20 mặc định / full không giới hạn — chọn ở SkillOptions), thứ tự Positive→Negative→Boundary→Edge→Security→UI/UX. `suite` (Smoke/Regression/New Feature/Exploratory) chỉ là metadata lưu+export, KHÔNG điều khiển logic (gating dùng `test_cases.stage`).
- **Import TC (0 token) + tối ưu token (2026-07-10)**: `parsePastedTestCases` (testcase-parser.js) parse bảng dán Excel/Sheets/Lark/CSV → `ImportTestCaseModal` → `handleImportTestCases` merge + `saveTestCasesApi` (KHÔNG gọi AI). `appendTestCases` chỉ gửi `{id,name,module}` của TC cũ (thay full JSON) để giảm input token. Phân biệt: "Thêm dòng" thủ công & "Import TC" = 0 token; "Bổ sung thêm"/trả lời câu hỏi & "Generate" = có token (AI viết nội dung).
...
| 2026-07-10 | **Sửa lỗi Bóc tách Feature thất bại do JSON Code Fences** — Cập nhật hàm `parseAiJson` trong `testcase-parser.js` để ưu tiên parse thô (`JSON.parse`) trước nếu dữ liệu bắt đầu bằng `[` hoặc `{`. Việc này ngăn Regex code fences bắt nhầm các khối block markdown bên trong dữ liệu JSON. | testcase-parser.js, HANDOFF.md, CODEBASE-STATE.md | ✅ Build pass (Vite) |
